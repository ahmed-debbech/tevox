...
func HookConsumers(consumers map[string]<-chan amqp091.Delivery) {
	...
	go ProcessTextToVoiceEventConsumer(consumers["A_QUEUE"], "A_QUEUE")

}

func ProcessTextToVoiceEventConsumer(feed <-chan amqp091.Delivery, queueName string) {
	...
	for msg := range feed {
		...
		if err := ProcessTextToVoiceEvent(msg.Body); err != nil {
			log.Println(err)
